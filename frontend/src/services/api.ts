...
export const updateWateringRecord = async (plantId: string, wateringId: string, payload: Record<string, any>): Promise<any> => {
  const res = await apiClient.put(`/user-plants/${plantId}/watering/${wateringId}`, payload)
  return res.data
}
